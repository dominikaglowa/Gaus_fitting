...
void RunFitEmiss(){
  ...
  TFile *_file0 = TFile::Open("Physisc_sn116_all.root"); // th input file
...
// his->Draw("");
 ...
 TF1 *fg2p2 = new TF1("fg2p2",Gaus2Gaus2,-80,40,4); //fit function
...
// fg2p2->FixParameter(3,0);
 ...
 double random_factor = 0.02; // From Dan:prompt width/random width
 h2dFile_pr->Add(h2dFile_r,-random_factor);

//do fits with free polynomial parameters
  for(Int_t i=hclaireA->GetNbinsX()-1;i>-1;i--){ //loop over all q bins
   hEmiss=NULL;
   ...
   //sprintf(name1,"pimissen_pr_DE_E_8_px",low,high);
 ...
 for(Int_t i=0;i<hclaireA->GetNbinsX();i++){
   hEmiss=NULL;
   low=(float)i/50;
   high=(float)(i+1)/50;
   sprintf(name1,"pimissen_q_4_%4.3f_%4.3f",low,high);
   //sprintf(name1,"pimissen_pr_DE_E_8",low,high);
...
void GetSignal(){
 ...
 //TH1D* his=((TH1D*)gDirectory->Get("DeltaE_Missmom50_BeamE8"));
  ...
  //  TH1D* his=((TH1D*)gDirectory->Get("pmissem_pr_DE_E_8"));
  his->SetNameTitle("hisb","hisb");
  his->Add(((TH1F*)gDirectory->Get("pimissen_q_4_0.460_0.480")));

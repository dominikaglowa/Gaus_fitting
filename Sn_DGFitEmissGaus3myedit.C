...
Double_t Gaus2Gaus2(Double_t *x, Double_t *par) { // 2 gaussian fit 
 //standard resolution function
  ...
  //standard fit
  //return TMath::Gaus(x[0],Gmean,Gwidth)*par[0]/(2.5066*Gwidth)+TMath::Gaus(x[0],par[1],par[2])*(par[3])/(2.5066*par[2]);//original line
 return TMath::Gaus(x[0],-3.87,7.32)*par[0]/(2.5066*Gwidth)+TMath::Gaus(x[0],par[1],par[2])*(par[3])/(2.5066*par[2]);
}
...
Double_t Gaus3(Double_t *x, Double_t *par) { // single gaussian fit for use near coherent maxima
  ...
  //cout<<"par0 "<<par[0]<<" par1 "<<par[1]<<" par2 "<<par[2]<<endl;
}
...
//do fits with free polynomial parameters
 ...
 myp->SetParameters(-24.3009,113.588,-158.613); // set initial parameters for the fits?
...
void GetSignal(){
  ...
  TF1 *fg3 = new TF1("fg3",Gaus3,-25,20,12);//original line
  //TF1 *fg3 = new TF1("fg3",Gaus3,-25,25,3);
  //Fix parameters to 0.5 value;
  fg3->SetParameters(1000,-2,5);//original line
  //fg3->SetParameters(400,-4,10);
